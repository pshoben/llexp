...
#include "queue_2w1r_boost_spsc_lockfree.h"
...
void * reader_thread_func( __attribute__((unused)) void * args ) 
{
    ...
    TimespecPair64 * samples = create_samples( g_max_samples );
    ...
    while( true ) {
      ...
      next_sample = g_queue.read0( next_sample ) ; 
      if( next_sample != current_sample && ( !( next_sample == nullptr )))
      {
         num_reads ++;
      }
      current_sample = next_sample;
      next_sample = g_queue.read1( next_sample ) ; 
      if( next_sample != current_sample && ( !( next_sample == nullptr )))
      {
         num_reads ++;
      }
...
void * writer_thread_func( __attribute__((unused)) void * args ) 
{
    ...
    {
        TimespecPair64 msg = g_queue.write_blocking( queue_id ) ;
        count++;
...
int main(int argc, char * const * argv) 
{
  ...
  if( g_verbose ) {
    std::cout << "queue 0 " << (g_queue.queue0.is_lock_free() ? " IS ": " IS NOT ") << "lockfree\n";
    std::cout << "queue 1 " << (g_queue.queue1.is_lock_free() ? " IS ": " IS NOT ") << "lockfree\n";
  ...
  g_stats = new MessageStats{ "spsc2to1 ", g_max_samples, (unsigned int) g_msg_per_sec, g_num_writers };
