...
#include <cstdio>
#include <unistd.h>
#include "queue_v2_pthread_spinlock_std_queue.h"
#include <stdint.h>
#include "common.h"
#include <algorithm>
//#include <atomic>
...
void start_threads( unsigned int num_threads, vector<std::thread> & threads, const char * msg, int is_reader )
{
  ...
  {
    // hold the global lock while creating the threads
    std::lock_guard<mutex> biglock(queue.iomutex);
  ...
  for (unsigned i = 0; i < num_threads; ++i) 
  {
    ...
    if( is_reader ) 
    {
       ...
       threads[i] = std::thread([ verbose, &queue.iomutex, i, msg, &queue, max_writes, stats ] {
       TimespecPair * samples ;
       {
          std::lock_guard<mutex> iolock(queue.iomutex);
...
////            }
      ...
      threads[i] = std::thread([ verbose, &queue.iomutex, i, msg, &queue, max_writes, msg_per_sec, target_delay ] {
...
//          }
  ...
  std::cout << "finished creating threads\n";
  }
